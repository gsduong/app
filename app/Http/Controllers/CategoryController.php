...
class CategoryController extends Controller
{
    ...
    public function index() {
    	return view('restaurant/menu/all', ['restaurant' => $this->restaurant]);
    }
    ...
    public function create(Request $request, $restaurant_slug){
    	...
    	if ($this->restaurant->categories->where('name', '=', $name)->first() === null) {
    		...
    		return redirect()->route('category.list', $restaurant_slug)->with('success', 'New category created!');
    	}
    	return redirect()->route('category.list', $restaurant_slug)->with('error', 'Duplicated category!');
    }

    public function update(Request $request, $restaurant_slug){
		$name = $request->name;
		if (!$name || !$request->id) {
			return redirect()->route('category.list', $restaurant_slug)->with('error', 'Category name required!');
		}
		...
		if ($category->name == $name) {
			...
			$category->save();
			return redirect()->route('category.list', $restaurant_slug)->with('success', 'Category updated!');
		}
		else {
